...
var { tiktokdl } = require('../lib/tt.js')
var { cerpen } = require('../lib/cerpen.js')
var {
	Searchnabi,
	NiatSholat,
	Gempa,
...
router.get('/ayatkursi', async (req, res, next) => {
		Apikey = req.query.apikey;
		if (!Apikey) return res.json(loghandler.notparam)
		if (listkey.includes(Apikey)){
		AyatKursi()
...
router.get('/random/asupan/ghea', async (req, res, next) => {
        var Apikey = req.query.apikey
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
  const ghea = JSON.parse(fs.readFileSync(__path +'/data/ghea.json'));
  const randghea = ghea[Math.floor(Math.random() * ghea.length)];
  data = await fetch(randghea).then(v => v.buffer())
  await fs.writeFileSync(__path +'/tmp/ghea.mp4', data)
  res.sendFile(__path +'/tmp/ghea.mp4')
} else {
res.json(loghandler.invalidKey)
}
})

router.get('/random/asupan/santuy', async (req, res, next) => {
        var Apikey = req.query.apikey
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
     ...
     tiktokdl(`${url}`)
        .then((result) => {
            res.json({
               creator: `${creator}`,
               Title: result.title,
               Video: result.media[1].url,
               Quality_video: result.media[1].quality,
               Size_video: result.media[1].formattedSize,
               Audio: result.media[2].url,
               Quality_audio: result.media[2].quality,
               Size_audio: result.media[2].formattedSize
             })
         })
         .catch((error) => {
            res.json(error);
